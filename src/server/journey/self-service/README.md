...
├── invalid/           # GET      /journey/self-service/invalid                               (ML-1286)
...
│                      # GET      /journey/self-service/c/{slug}/continue/{...}                (ML-1166, ML-1167)
├── outcome-document/  # GET      /journey/self-service/outcome-document/{snapshotSlug}        (ML-1306)
├── schemas/           # slug Joi schema shared across slug-bearing routes
├── data/              # self-service.json + load-time parser/sanitiser
└── services/          # journey-data, journey-router, journey-answer-log,
                       # load-iat-context, application-handoff,
                       # data-quality, data-quality-init, sanitise
```
...
| GET    | `/journey/self-service/invalid`                                              | "Your session has timed out" page — answers saved 24h, with a "Return to start" link                                                                                                                                                                                             | `invalid/`          |
...
| GET    | `/journey/self-service/c/{slug}/continue/{outcomeTypeId}/{outcomePath*}`     | **Mint** a snapshot, then hand off: outcomeTypes with `overrideCtaButtonUrl` 302 to the exemption journey (ML-1166); outcomeTypes with `module` 302 to MCMS (ML-1167). Answers (and the snapshot `answersUrl`) are passed as a query string. 404 if the outcomeType has neither. | `outcome/`          |
...
`view-answers` redirects to the public snapshot page; `continue` hands off
to one of two destinations, decided by the focused outcomeType:

- an outcomeType with an `overrideCtaButtonUrl` hands off into the Defra
  exemption application journey (ML-1166), redirecting to that URL;
- an outcomeType with a `module` hands off to MCMS (ML-1167), redirecting
  to the configured `mcms.url` + `mcms.path`.

In both cases the answers (and the snapshot `answersUrl`) ride along as a
query string. An outcomeType with **neither** `overrideCtaButtonUrl` nor
`module` is not a valid `continue` target and returns 404.
...
sequenceDiagram
    ...
    rect rgb(232,245,255)
        ...
        API-->>F: 201 { slug: snapSlug, viewUrl, answersUrl }
...
| `/iat-contexts/{slug}/outcome-documents` | POST   | **Mint** a new immutable snapshot from the context + payload. Returns `{slug, viewUrl, answersUrl, snapshot}` (the handoff consumes `answersUrl`). |
...
| `iat.inFlightTtlMs`              | `IAT_IN_FLIGHT_TTL_MS`    | 24h     | **Backend config** (`marine-licensing-backend/src/config.js`), listed here because the frontend lifecycle depends on it — it is _not_ a frontend flag. Sets the `expiresAt` value written on context creation; Mongo's TTL monitor deletes the doc that many ms after that timestamp. |
