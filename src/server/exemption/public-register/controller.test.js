...
describe('#publicRegister', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(apiClient, 'patchToBackend').mockResolvedValue({
      payload: {
        value: {
          id: mockExemption.id,
  ...
  describe('#publicRegisterSubmitController', () => {
    ...
    test('Should show error messages with invalid data', async () => {
      const apiPostMock = jest.spyOn(apiClient, 'patchToBackend')
      apiPostMock.mockRejectedValueOnce({
        message: 'Backend API call failed: Validation error',
        data: {
          payload: {
            validation: {
              source: 'payload',
    ...
    test('Should correctly set the cache when submitting public register', async () => {
      ...
      await publicRegisterSubmitController.handler(mockRequest, h)
